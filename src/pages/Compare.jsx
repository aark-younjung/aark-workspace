...
// 分數顯色 — 暗色版用較亮的色階（400 系列），與其他 audit 頁一致
...
// 每個網站的主色（最多 4 個）— 暗色版用半透明底色 + 亮邊框
const SITE_COLORS = [
  { border: 'border-orange-400/60', glow: 'bg-orange-500/15', badge: 'bg-orange-500',  text: 'text-orange-300',  bar: 'bg-orange-400',  chipBg: 'bg-orange-500/15' },
  { border: 'border-blue-400/60',   glow: 'bg-blue-500/15',   badge: 'bg-blue-500',    text: 'text-blue-300',    bar: 'bg-blue-400',    chipBg: 'bg-blue-500/15'   },
  { border: 'border-violet-400/60', glow: 'bg-violet-500/15', badge: 'bg-violet-500',  text: 'text-violet-300',  bar: 'bg-violet-400',  chipBg: 'bg-violet-500/15' },
  { border: 'border-emerald-400/60',glow: 'bg-emerald-500/15',badge: 'bg-emerald-500', text: 'text-emerald-300', bar: 'bg-emerald-400', chipBg: 'bg-emerald-500/15'},
]
    ...
    <PageBg>
      ...
      <div className="relative z-10">
        <main className="max-w-6xl mx-auto px-6 py-12">
          ...
          {/* URL 輸入區 */}
          <div className="bg-black/40 backdrop-blur-md rounded-2xl border border-white/10 p-8 mb-10">
              ...
              {urls.map((url, i) => (
                <div key={i} className="flex gap-2 items-center">
                  ...
                  <input
                    ...
                    disabled={isLoading}
                    className="flex-1 px-4 py-3 rounded-xl bg-black/40 border border-white/15 text-white placeholder-white/30 focus:outline-none focus:ring-2 focus:ring-orange-400 text-sm disabled:opacity-50"
                  />
                  {urls.length > 2 && (
                    <button onClick={() => removeUrl(i)} disabled={isLoading}
                      className="text-white/40 hover:text-red-400 transition-colors text-lg disabled:opacity-30">
                      ✕
                    </button>
                  )}
                </div>
            ...
            <div className="flex items-center gap-4 flex-wrap">
              {urls.length < MAX_URLS && (
                <button onClick={addUrl} disabled={isLoading}
                  className="px-4 py-2 bg-white/5 border border-white/15 text-white/70 hover:text-white hover:bg-white/10 rounded-lg text-sm transition-colors disabled:opacity-30">
                  + 新增競品
                </button>
              )}
              {!isPro && urls.length >= MAX_URLS && (
                <Link to="/pricing" className="px-4 py-2 bg-orange-500/15 border border-orange-400/40 text-orange-300 rounded-lg text-sm hover:bg-orange-500/25 transition-colors">
                  🔒 升級 Pro 比較最多 4 個網站
                </Link>
              )}
              <button onClick={handleCompare} disabled={isLoading}
                className="px-8 py-3 bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-white font-semibold rounded-xl disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg shadow-orange-500/30">
                {isLoading ? (
              ...
              <div className="flex flex-wrap gap-3 mt-5">
                {loadingStates.map((state, i) => (
                  <div key={i} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm border ${
                    state === '完成' ? 'bg-green-500/10 border-green-500/30 text-green-300' :
                    state === '失敗' ? 'bg-red-500/10 border-red-500/30 text-red-300' :
            ...
            <>
              ...
              <div className={`grid gap-4 mb-10 ${results.length === 2 ? 'sm:grid-cols-2' : results.length === 3 ? 'sm:grid-cols-3' : 'sm:grid-cols-4'} grid-cols-1`}>
                {results.map((r, i) => {
                  ...
                  return (
                    <div key={i} className={`relative rounded-2xl border-2 ${totalWinners[i] ? 'border-yellow-400/70 shadow-lg shadow-yellow-400/20' : c.border} bg-black/40 backdrop-blur-md overflow-hidden`}>
                      {/* 頂部色條 */}
                      ...
                      {results.map((r, i) => {
                        ...
                        return (
                          <div key={i} className="flex justify-center">
                            <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full ${c.chipBg} ${c.text} truncate max-w-full`}>
                      ...
                      return (
                        <div key={key}
                          ...
                          {vals.map((v, i) => (
                            <div key={i} className="flex justify-center">
                              {v
                                ? <span className={`inline-flex items-center justify-center w-7 h-7 rounded-full bg-green-500/20 text-green-300 text-sm font-bold border border-green-500/30`}>✓</span>
                                : <span className="inline-flex items-center justify-center w-7 h-7 rounded-full bg-white/5 text-white/30 text-sm border border-white/10">✗</span>
                              }
                            </div>
                          ))}
                        </div>
