...
export default function GSCReport() {
  ...
  if (loading) return (
    <div className="min-h-screen flex items-center justify-center" style={bg}>
      <div className="text-center">
        ...
        <p className="text-white/80">載入 GSC 數據中…</p>
      </div>
    </div>
  )
  ...
  if (error === 'NOT_AUTHENTICATED') return (
    <div className="min-h-screen flex items-center justify-center" style={bg}>
      <div className="bg-white/12 backdrop-blur-md rounded-2xl p-8 max-w-sm text-center border border-white/20">
        ...
        <h2 className="text-xl font-bold text-white mb-2">需要 Google 授權</h2>
        <p className="text-white/80 text-sm mb-6">請先在儀表板連結 Google 帳號，才能查看 Search Console 數據。</p>
        <div className="flex gap-3 justify-center">
          <Link to={`/dashboard/${id}`} className="px-4 py-2 bg-orange-500 text-white rounded-xl text-sm hover:bg-orange-600">
            返回儀表板
          </Link>
          <button onClick={initiateGoogleAuth} className="px-4 py-2 bg-white/12 border border-white/20 rounded-xl text-sm text-white hover:bg-white">
            連結 Google
          </button>
        </div>
      </div>
  ...
  if (error === 'NO_SITE_URL') return (
    <div className="min-h-screen flex items-center justify-center" style={bg}>
      <div className="bg-white/12 backdrop-blur-md rounded-2xl p-8 max-w-sm text-center border border-white/20">
        <div className="text-4xl mb-4">🌐</div>
        <h2 className="text-xl font-bold text-white mb-2">尚未設定 GSC 網站</h2>
        <p className="text-white/80 text-sm mb-6">請在儀表板設定 Search Console 網站網址（例：sc-domain:yourdomain.com）。</p>
        <Link to={`/dashboard/${id}`} className="px-4 py-2 bg-orange-500 text-white rounded-xl text-sm hover:bg-orange-600">
          返回儀表板設定
        </Link>
      </div>
    ...
    <div className="min-h-screen" style={bg}>
      ...
      <div className="relative max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-3 mb-6">
          <Link to={`/dashboard/${id}`} className="text-white/60 hover:text-white text-sm flex items-center gap-1">
            ← 返回儀表板
          </Link>
        </div>
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <h1 className="text-2xl font-bold text-white">Google Search Console 詳細報告</h1>
              <p className="text-white/60 text-sm">{website?.url || summary.siteUrl} · 過去 30 天</p>
            </div>
          </div>
          {highRecs > 0 && (
        ...
        <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-5 mb-8">
          <h3 className="font-semibold text-white mb-4">SEO 健康指標</h3>
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6">
            ...
            {devicePie.length > 0 && (
              <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-5">
                <h3 className="font-semibold text-white mb-4">搜尋裝置分佈</h3>
                    ...
                    {devicePie.map((d, i) => (
                      <div key={d.name} className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-white capitalize">{d.name}</span>
                        </div>
                        <div className="text-right">
                          <span className="font-semibold text-white">{d.pct}%</span>
                          <span className="text-xs text-white/60 ml-2">({fmt(d.value)} 點擊)</span>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            {opportunities.length > 0 && (
              <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-5">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="font-semibold text-white">🎯 快速突破機會（排名 4–15）</h3>
                  <span className="text-xs bg-green-100 text-green-700 px-2 py-1 rounded-full">{opportunities.length} 個關鍵字</span>
                </div>
                <p className="text-sm text-white/60 mb-4">這些關鍵字離第 1 頁只差一步，優化它們可快速提升流量。</p>
                  ...
                  {opportunities.slice(0, 5).map((q, i) => (
                    <div key={i} className="flex items-center gap-3 p-3 bg-white/10 rounded-xl">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-sm font-medium text-white truncate">{q.query}</div>
                        <div className="text-xs text-white/60">曝光 {fmt(q.impressions)} 次 · 點擊 {fmt(q.clicks)} 次</div>
                      </div>
                      <div className="text-right">
                        <div className="text-sm font-bold text-amber-600">#{q.position.toFixed(0)}</div>
                        <div className="text-xs text-white/60">目前排名</div>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        {activeTab === 'trend' && (
          <div className="space-y-6">
            {summary.timeline.length > 0 ? (
              <>
                <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-5">
                  <h3 className="font-semibold text-white mb-4">點擊 & 曝光趨勢</h3>
                ...
                <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-5">
                  <h3 className="font-semibold text-white mb-4">CTR & 平均排名趨勢</h3>
                  ...
                  <p className="text-xs text-white/60 mt-2 text-center">平均排名：數字越小代表排名越好（右側 Y 軸已翻轉）</p>
                </div>
              </>
            ) : (
              <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-10 text-center text-white/60">
          ...
          <div className="space-y-6">
            ...
            {opportunities.length > 0 && (
              <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-5">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <h3 className="font-semibold text-white">快速突破機會關鍵字</h3>
                  <span className="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded-full">{opportunities.length} 個</span>
                </div>
                <p className="text-sm text-white/60 mb-4">排名 4–15 名，稍加優化即可進入前 3 名帶來大量流量</p>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-white/15">
                        <th className="text-left py-2 text-white/60 font-medium">關鍵字</th>
                        <th className="text-right py-2 text-white/60 font-medium">排名</th>
                        <th className="text-right py-2 text-white/60 font-medium">曝光</th>
                        <th className="text-right py-2 text-white/60 font-medium">點擊</th>
                        <th className="text-right py-2 text-white/60 font-medium">CTR</th>
                      </tr>
                    </thead>
                    <tbody>
                      {opportunities.map((q, i) => (
                        <tr key={i} className="border-b border-slate-100 hover:bg-white/10">
                          ...
                          <td className="text-right py-2.5 text-white/80">{fmt(q.impressions)}</td>
                          <td className="text-right py-2.5 text-white/80">{fmt(q.clicks)}</td>
                          <td className="text-right py-2.5 text-white/80">{(q.ctr * 100).toFixed(1)}%</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            {lowCTR.length > 0 && (
              <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-5">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-lg">🔤</span>
                  <h3 className="font-semibold text-white">高曝光但點擊率低落的關鍵字</h3>
                  <span className="text-xs bg-red-100 text-red-700 px-2 py-0.5 rounded-full">{lowCTR.length} 個</span>
                </div>
                <p className="text-sm text-white/60 mb-4">這些關鍵字有大量曝光機會，但標題或描述不夠吸引人，需要優化</p>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-white/15">
                    ...
                    <tbody>
                      {lowCTR.slice(0, 10).map((q, i) => (
                        <tr key={i} className="border-b border-slate-100 hover:bg-white/10">
                          ...
                          <td className="text-right py-2.5 text-white/80">#{q.position.toFixed(0)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            {sortedKWs.length > 0 && (
              <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-5">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="font-semibold text-white">所有關鍵字（前 20 名）</h3>
                  <div className="flex gap-1">
                    {['clicks', 'impressions', 'position'].map(k => (
                      <button
                        key={k}
                        onClick={() => setKwSort(k)}
                        className={`px-3 py-1 rounded-lg text-xs ${kwSort === k ? 'bg-orange-500 text-white' : 'bg-white/12 text-white/80 hover:bg-white'}`}
                      >
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {sortedKWs.map((q, i) => (
                        <tr key={i} className="border-b border-slate-100 hover:bg-white/10">
                          <td className="py-2.5 text-white/60 text-xs">{i + 1}</td>
                          <td className="py-2.5 font-medium text-white max-w-[200px] truncate">{q.query}</td>
                          <td className="text-right py-2.5 text-white/80">{fmt(q.clicks)}</td>
                          <td className="text-right py-2.5 text-white/80">{fmt(q.impressions)}</td>
                          <td className="text-right py-2.5 text-white/60">{(q.ctr * 100).toFixed(1)}%</td>
                          <td className="text-right py-2.5">
                            <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                              q.position <= 3 ? 'bg-green-100 text-green-700' :
                              q.position <= 10 ? 'bg-blue-100 text-blue-700' :
                              q.position <= 20 ? 'bg-amber-100 text-amber-700' : 'bg-white/5 text-white/60'
        ...
        {activeTab === 'pages' && (
          <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-5">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold text-white">熱門頁面（按自然搜尋）</h3>
              <div className="flex gap-1">
                {['clicks', 'impressions', 'position'].map(k => (
                  <button
                    key={k}
                    onClick={() => setPageSort(k)}
                    className={`px-3 py-1 rounded-lg text-xs ${pageSort === k ? 'bg-orange-500 text-white' : 'bg-white/12 text-white/80 hover:bg-white'}`}
                  >
                    {k === 'clicks' ? '點擊' : k === 'impressions' ? '曝光' : '排名'}
                  </button>
                ))}
            ...
            {sortedPages.length === 0 ? (
              <div className="text-center text-white/60 py-8">暫無頁面數據</div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-white/15">
                      <th className="text-left py-2 text-white/60 font-medium">#</th>
                      <th className="text-left py-2 text-white/60 font-medium">頁面路徑</th>
                      <th className="text-right py-2 text-white/60 font-medium">點擊</th>
                      <th className="text-right py-2 text-white/60 font-medium">曝光</th>
                      <th className="text-right py-2 text-white/60 font-medium">CTR</th>
                      <th className="text-right py-2 text-white/60 font-medium">排名</th>
                  ...
                  <tbody>
                    {sortedPages.map((p, i) => (
                      <tr key={i} className="border-b border-slate-100 hover:bg-white/10">
                        ...
                        <td className="text-right py-2.5 font-medium text-white">{fmt(p.clicks)}</td>
                        <td className="text-right py-2.5 text-white/80">{fmt(p.impressions)}</td>
                        <td className="text-right py-2.5 text-white/60">{(p.ctr * 100).toFixed(1)}%</td>
                        <td className="text-right py-2.5">
                          <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
        ...
        {activeTab === 'recs' && (
          <div className="space-y-4">
            <p className="text-sm text-white/60">根據你的 GSC 數據，以下是優先建議（高優先 → 中優先）：</p>
            {recs.length === 0 && (
              <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-10 text-center text-white/60">
                <div className="text-4xl mb-3">🎉</div>
                <p>目前找不到明顯問題，繼續保持！</p>
            ...
            {recs.map((rec, i) => (
              <div
                key={i}
                className={`bg-white/10 backdrop-blur-md border rounded-2xl p-5 ${
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-white/80 mb-3 whitespace-pre-line">{rec.detail}</p>
                    <div className="space-y-1.5">
                      {rec.actions.map((a, j) => (
                        <div key={j} className="flex items-start gap-2 text-sm text-white">
