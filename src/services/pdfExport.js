...
function buildClientProposalHTML(data, clientInfo) {
  const { website, seoAudit, aeoAudit, geoAudit, eeatAudit } = data
  ...
  const actionItems = buildActionItems({ seoAudit, aeoAudit, geoAudit, eeatAudit })

  // 共用 checks 列表（跟舊版 buildReportHTML 同邏輯）
  const seoChecks = [
  ...
  function renderChecks(checks, color) {
    return checks.map(c => `
      <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid #f1f5f9;">
        <span style="font-size:13px;font-weight:bold;color:${c.passed ? '#16a34a' : '#dc2626'};width:20px;text-align:center;">
          ${c.passed ? '✓' : '✗'}
        </span>
        <span style="font-size:13px;color:#374151;flex:1;">${c.name}</span>
  ...
  function renderScoreCard(label, score, color) {
    return `
      <div style="flex:1;text-align:center;padding:16px;background:#f8fafc;border-radius:12px;border:2px solid ${color}33;">
        <div style="font-size:32px;font-weight:bold;color:${scoreColor(score)};">${score}</div>
  ...
  return `
    <div style="width:794px;background:white;font-family:'Microsoft JhengHei','微軟正黑體','PingFang TC',sans-serif;color:#1e293b;padding:0;">
      ...
      <!-- ═════ 封面 / Cover Page ═════ -->
      <div style="background:linear-gradient(155deg, #050608 0%, #04130f 30%, #052e2c 60%, #18c590 130%);padding:80px 56px;color:white;min-height:700px;display:flex;flex-direction:column;">
        ...
        <div style="font-size:24px;color:rgba(255,255,255,0.75);margin-bottom:50px;letter-spacing:-0.01em;">
      ...
      <!-- ═════ 第 1 頁：總體分數 + LLMO 5 訊號層說明 ═════ -->
      <div style="padding:48px 56px 32px;background:white;">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:12px;">
          ...
          <tbody>
            <tr>
              <td style="padding:10px 12px;border-bottom:1px solid #f1f5f9;color:${LAYER_COLOR.SEO};font-weight:600;">① SEO</td>
              <td style="padding:10px 12px;border-bottom:1px solid #f1f5f9;color:#475569;">傳統搜尋排名地基 — 讓 Google 找到你</td>
              <td style="padding:10px 12px;border-bottom:1px solid #f1f5f9;text-align:right;font-weight:bold;color:${scoreColor(seoScore)};">${seoScore}</td>
            </tr>
            <tr>
              <td style="padding:10px 12px;border-bottom:1px solid #f1f5f9;color:${LAYER_COLOR.AEO};font-weight:600;">② AEO</td>
              <td style="padding:10px 12px;border-bottom:1px solid #f1f5f9;color:#475569;">讓 AI 把你當答案、引用你的內容</td>
              <td style="padding:10px 12px;border-bottom:1px solid #f1f5f9;text-align:right;font-weight:bold;color:${scoreColor(aeoScore)};">${aeoScore}</td>
            </tr>
            <tr>
              <td style="padding:10px 12px;border-bottom:1px solid #f1f5f9;color:${LAYER_COLOR.GEO};font-weight:600;">③ GEO</td>
              <td style="padding:10px 12px;border-bottom:1px solid #f1f5f9;color:#475569;">讓 ChatGPT / Perplexity 主動推薦你</td>
              <td style="padding:10px 12px;border-bottom:1px solid #f1f5f9;text-align:right;font-weight:bold;color:${scoreColor(geoScore)};">${geoScore}</td>
            </tr>
            <tr>
              <td style="padding:10px 12px;border-bottom:1px solid #f1f5f9;color:${LAYER_COLOR['E-E-A-T']};font-weight:600;">④ E-E-A-T</td>
              <td style="padding:10px 12px;border-bottom:1px solid #f1f5f9;color:#475569;">可信度框架 — AI 判斷你值得引用的訊號</td>
              <td style="padding:10px 12px;border-bottom:1px solid #f1f5f9;text-align:right;font-weight:bold;color:${scoreColor(eeatScore)};">${eeatScore}</td>
            </tr>
            <tr>
              <td style="padding:10px 12px;color:#f97316;font-weight:600;">⑤ aivis</td>
              <td style="padding:10px 12px;color:#475569;">跨 LLM 引用率追蹤 — 結果驗證層</td>
              <td style="padding:10px 12px;text-align:right;color:#94a3b8;font-size:11px;">需 Pro 訂閱</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div style="padding:32px 56px;background:white;page-break-before:always;">
      ...
      <div style="padding:32px 56px;background:white;page-break-before:always;">
        ...
        ${[
          ...
          <div style="margin-bottom:32px;">
            ...
            ${renderChecks(s.checks, LAYER_COLOR[s.key])}
          </div>
        `).join('')}
      </div>

      <!-- ═════ Footer ═════ -->
      <div style="padding:24px 56px 32px;background:#f8fafc;border-top:1px solid #e2e8f0;text-align:center;">
        <div style="font-size:11px;color:#64748b;line-height:1.8;">
          ${agencyName ? `本報告由 <strong style="color:#1e293b;">${agencyName}</strong> 提供 · ` : ''}
          技術監測 · <strong style="color:#18c590;">Aark</strong> AI 雷達（LLMO 監測平台）<br/>
          報告生成 · ${date} · 結果僅供參考、實際 AI 搜尋排名受多重因素影響
        </div>
      </div>
...
export async function exportClientProposalPDF(data, clientInfo) {
  ...
  container.innerHTML = buildClientProposalHTML(data, clientInfo)
