...
export default async function handler(req, res) {
  ...
  try {
    // 並行 7 查（4 個 audit 表 + 3 個 aivis 表），head:true 只回 count 不抓 row 資料
    const [brandsRes, seoRes, aeoRes, geoRes, eeatRes, mentionsRes, scansRes] = await Promise.all([
    ...
    return res.status(200).json({ brands, reports, mentions, scans })
  } catch (err) {
