...
「監測題目」分頁按執行掃描，掃完之後是 `window.location.reload()` **重載同一個分頁**——題目清單長得一模一樣，結果其實在「監測總覽」。用戶回報「會讓人誤會沒有反應」，完全正確。

改成掃完 1.2 秒後導到 `/app/:id/visibility/overview`。**刻意用整頁導轉而不是 SPA 的 navigate**：AppVisibility 只在 `websiteId` 變動時重抓資料，同頁換 tab 不會 refetch，用 SPA 導過去會看到掃描前的舊數字——那比「沒反應」更糟。

（順帶查了其他地方：AppOverview 掃完也是 reload，但那頁本身就是結果頁、原地重載沒問題，不動。）

**二、Archive.org 佐證「實際上多久沒動過」**

起因是用戶丟 public-apis 清單來問。整份清單對我們幾乎沒用（通用目錄、英語圈為主），但 Archive.org 是少數真的能用的——而且補的是我們前一天才做的新鮮度指標的弱點。

`lastmod` 讀的是網站**自己宣稱**的更新時間，而很多 SEO 外掛每次載入就把它寫成當下，於是每頁看起來都「今天剛更新」。拿那種數字跟客戶說內容很新鮮，是在幫他自我感覺良好。

做法：查 CDX API 拿歷史快照，每筆帶內容雜湊（digest）。從最新往回走，digest 相同代表內容沒變——第一次出現這個 digest 的時間就是「內容最後一次真的改變」。這是**第三方留存的證據**，不是網站的自我宣告。

只在「宣稱 30 天內更新、但存檔顯示至少半年沒動」時才在免費掃描結果卡跳提示（`waybackNotice`，形狀比照 `renderModeNotice`）。其他情況一律不顯示——沒落差時多講一句都是雜訊。

**刻意不計分**：Archive.org 對台灣中小企業網站收錄很不平均。實測 `a-ark.com.tw` 有 2019 年起 28 份快照，`aark-workspace.vercel.app` **一份都沒有**。拿收錄狀況扣客戶的分完全站不住腳。

**踩到的坑（都是真實資料抓出來的，假資料測不到）**

1. ⚠️ **把資料空窗期當成「確認沒變」**。第一版用「現在 − 內容變動時間」算沒變多久。但 `a-ark.com.tw` 最後一次存檔在 87 天前——那 87 天發生什麼事我們根本沒有資料。改成用「最後存檔 − 變動時間」，並加上：最後存檔超過 120 天就不拿來反駁客戶的宣稱（`archive_outdated`）。**用過期資料指控客戶說謊是最糟的錯。**
2. ⚠️ **「2026-06-09 到 2026-06-09 之間（0 天）內容沒有實質變動」**——目前版本只被存檔一次時，第一版會生出這種廢話。加 `currentVersionSnapshots`，只有 1 份就回 `single_capture` 直說「無法判斷」。
3. Archive.org 中途真的掛了，回傳 `Internet Archive: Temporarily Offline` 的 **HTML 而不是 JSON**。這剛好驗證了 `JSON.parse` 失敗 → `unknown` → 不顯示的失效路徑是對的（已有對應單測）。
4. **import 不能省副檔名**：`geoAnalyzer.js` 有單測會被 node 直接載入，Vite 解析得了 `'./waybackFreshness'`，node 的 ESM loader 不行。專案其他地方省副檔名沒事是因為那些檔案只走 Vite。

**架構選擇**：走既有的 `/api/fetch-url` 代理查 CDX，**不新增 Vercel function**（Hobby 12 支上限）。逾時 8 秒、與三個子資源探測平行發車（archive.org 是不同主機、不受既有節流限制），幾乎不拉長掃描時間。任何失敗都回 unknown、不影響評分。

**驗證**：新增 18 條單測，全專案 **118/118 通過**；並用 `a-ark.com.tw` 的真實 CDX 資料跑過端到端（上面兩個坑就是這樣抓到的）。
...
- 卡片走「你的網站擋下了我們的爬蟲」分類（該網址回 `antiBotBlocked:true`，分類正確）
- 「改掃首頁 →」實心橘鈕、**點下去真的成功掃到 `httpbin.org`**——這條「掃內頁失敗 → 一鍵退回首頁」的路徑端到端驗證通過
- 「重試」灰框停用態 + 倒數（54 秒後可用）正常
- email 輸入框有邊框、「通知我」橘鈕正常

**中途的插曲值得記**：用戶第一次重整看到的還是舊版（按鈕沒 padding、輸入框沒邊框），一度以為修正沒生效。實際上線上 CSS／JS 都已是新版（直接抓 `assets/index-*.css` 與 `.js` grep 確認有 `.hl-fail .hl-btn` padding、`查不到這個網域`、`dnsFailed`）——問題出在瀏覽器快取的 `index.html` 仍指向舊的資產檔名，一般重整救不回來。**之後驗收前端改動一律先請用戶 `Ctrl+Shift+R` 或開無痕視窗**，並在懷疑「是不是沒部署」時先 grep 線上資產、不要憑畫面下結論。

**仍未驗**：email 送出那條（要等 `scan_leads` 建表 SQL 跑完，現在會落到 `error_logs` 的 `scan_lead_fallback`）；DNS 失敗卡的實機外觀（後端 API 已確認回 `dnsFailed:true`）。
...
這正是 2026-08-13 用戶對 `/app` 側也回報過一次的同款問題（WORKLOG「appshell.css 全表字級一階上調對齊經典版 14px 正文基準」），這次首頁沒跟著做過。**沿用同一套手法**：13.5→14.5 / 13→14 / 12.5→13.5，逐條檢查 32 處小字級——凡是「使用者要讀的內容」（狀態列、判語、FAQ 答案、三大差異化說明、排行榜/早鳥文案、頁尾連結等 16 處）才升級；凡是徽章／mono 小標／從屬數字（如分數卡「/100」分母、判級 pill、mono 時間戳）維持原樣不動（16 處），跟當時「徽章與 mono 小標 ≤11px 刻意不動」的判準一致，不是無腦全部放大。

新版首頁「/」的正文基準對齊到 appshell.css 已經驗證過的 14px。[lp-sage.css](src/styles/lp-sage.css)（廣告落地頁 A 組）本身底色是 16-17px，比這個基準還大，沒有一起動。

**驗證**：CSS 大括號平衡；dev server 確認編譯後的 CSS 正確帶有 12 處 14.5px＋10 處 14px（跟本機檔案數字一致）。⚠️ 沒有瀏覽器實機比對放大後的視覺，麻煩驗收時直接感受一下差異有沒有到位。
...
**修法**：[AppShell.jsx](src/components/appshell/AppShell.jsx) 的 `.as-brand`（logo + 「AI 雷達」字樣）從 `<div>` 改成 `<Link to="/">`，補 `aria-label="回首頁"`；CSS 加 hover 灰底 + focus-visible 外框，讓它讀起來像個可點的東西，不是純裝飾。「＋ 新增網站」那顆連結繼續保留（不衝突，兩個入口並存：一個是通用直覺 logo、一個是明講功能的文字連結）。手機版沿用既有邏輯不顯示 logo（該版面本來就把它藏起來省空間，手機仍有「＋ 新增網站」nav item 可用）。

**驗證**：eslint 0 問題；CSS 大括號平衡；dev server 這次連上，確認 `aria-label="回首頁"` 有正確出現在編譯後的模組裡。
...
### 2026-08-13s（新版字級整體上調 — 已部署，用戶回饋字偏小）

appshell.css 全表字級一階上調對齊經典版 14px 正文基準（13.5→14.5/13→14/12.5→13.5/12→13/11.5→12.5，共 79 處）；徽章與 mono 小標（≤11px）刻意不動。

### 2026-08-13r（🟡 第二批 #4：內容任務單 — 已部署）

AppGap 每個缺口題加「📋 內容任務單」摺疊卡（[AppGap.jsx](src/components/appshell/AppGap.jsx) `taskBriefMarkdown`/`TaskBrief`）：目標/搜尋意圖（問句字面規則、透明標注判法）/建議標題（問句直接當標題）/內容要求（首段直答/H2H3/佐證）/建議 Schema（FAQPage+Article）/AI 目前引用誰（真實 others 名單）/完成後重掃驗證——**一鍵複製 markdown**、可直接交給寫手或客戶（代理商派工交付物）。誠實：模板組裝非 AI 文章（卡上有標注），守住三方定案的「任務單而非成稿」邊界。

### 2026-08-13q（觀察名單網址防呆 — 已部署）

實案：用戶把三個競品填成官網「網址」→ 比對必 0%（AI 回答寫品牌名、不寫網址）。修：`looksLikeUrl` 偵測（http 開頭或 .com/.tw 等 TLD）→ ①編輯器 chip 轉琥珀警告色 ②編輯器下方警告列（附正確示範）③比較列網址型 0% 項附「請改品牌名」提示。比對系統本身正常（DBG 18%＝21/119 真實提及）。

### 2026-08-13p（🟡 第二批 #3：品牌資料完整度 — 已部署）

AppVisibility 總覽分頁尾端新進度條卡：四欄位（網域/產業別/品牌描述/競品名單）各講「為什麼要填」（引用判定/題庫準度/同題比較），缺項各附直達補填入口（競品→分頁、其餘→經典版品牌管理）；100% 顯示齊全確認。只算 schema 真有的欄位、不畫大餅（Kuroma 檔案完整度的誠實版）。

### 2026-08-13o（🟡 第二批 #2：AI 曝光監測分頁化 — 已部署，用戶提案）

用戶點出區塊太多（KPI/矩陣/分軸/競品/來源/觀感/事實七塊一頁到底）→ 比照網站體檢分頁化：**總覽**（KPI+矩陣+另計分軸）/**競品比較**（同題比較+名單編輯）/**引用來源**（誰在影響 AI+品牌外部提及）/**AI 怎麼說你**（原話摘引+AI 講錯你）。真 URL 深連結 `/app/:id/visibility/:visTab`＋無效導 overview（比照 AppHealth 契約）；期間切換與「管理/重新掃描」留頁首跨分頁通用；分頁樣式沿用 as-health-tabs；四分頁全是既有內容、不蓋空殼（「分頁跟著功能長」原則兌現）。KPI 卡「設定→」錨點改分頁連結。

### 2026-08-13n（🟡 第二批 #1：AI 怎麼描述你（觀感 Lite v1）— 已部署）

[aivisData.js](src/components/appshell/aivisData.js) `buildBrandVoice`＋AppVisibility 新區塊：逐引擎摘引「品牌題回答」原話（優先含品牌名的句子、最多 160 字）＋`<details>` 展開完整回答查驗（Codex 反黑箱要求）。誠實邊界：**v1 不做機器情緒判定**（keyword 猜情緒會冤枉 AI）——只呈現「AI 真的說了什麼」；情緒/一致性標註等後端 LLM 彙整功能再上（foot 有明示）。這是 7/21 報告「Kuroma 觀感頁 80% 價值、成本極低」的落地。
...
1. **頁型判斷** — 新 [src/lib/pageAudit.js](src/lib/pageAudit.js)（`isHomepage` / `HOMEPAGE_NOTES`）。首頁缺麵包屑/FAQ 是正常的（首頁在最上層、FAQ 在 FAQ 頁）→ **N/A：不紅字、不扣分**。**第二刀（用戶重掃發現只改文字不夠）**：[aeoAnalyzer.js](src/services/aeoAnalyzer.js) `scoredPassed` 首頁對這兩項**免扣分**（DB raw boolean 照實存＝誠實、只有 score 用調整後）；顯示層 [AEOAudit.jsx](src/pages/AEOAudit.jsx) + 改版 [healthData.js](src/components/appshell/healthData.js) 同步翻 `passed=true` + 正常化說明。**舊 audit 要重掃才套新計分。**例外：首頁真有 FAQ 區塊（`faq_visual`）＝該補 schema 真問題，照舊扣分。
...
6. **快取時間差診斷（DP Meta 84→72 字「改了沒變」）** — 用戶改完 Meta 重掃仍 84 字。驗證：帶查詢參數繞過快取抓到**新 72 字**（範圍內），一般抓法仍是 LiteSpeed `00:52` 舊快取頁 → 改有存、快取未清。指引：後台 LiteSpeed 快取→清除全部→重掃。

7. **快取新鮮度提示（6 的產品化，已做）** — 新 [src/lib/cacheDetect.js](src/lib/cacheDetect.js)（LiteSpeed / WP Super Cache / W3TC / WP Rocket / WP Fastest Cache 五種 HTML 註記指紋 + 生成時間解析）+ [CacheFreshnessNote.jsx](src/components/CacheFreshnessNote.jsx)（深/亮兩色、即時抓當下頁、同 URL session 內只抓一次）。快取齡 ≥60 分鐘才顯示「⏱ 這一頁由 X 供應快取版（N 小時前生成），剛改過請先清快取再掃」；清完快取重整自動消失。文案守誠實立場：快取不是錯誤（AI 爬蟲讀到的也是這份）。接 SEO/AEO/GEO/EEAT 四詳情頁（深）+ 改版 AppHealth（亮）；規則寫進 AGENTS.md §0.1 第 4 條。
...
**依訪客分組檢視**：散著看只是一堆網址，看不出高價值訊號。加 `anonVisitorGroups` useMemo（依 `session_id` 收攏，無 id 的舊資料歸「未識別」並永遠沉底，其餘依最近活動排序）＋ 檢視切換鈕（⏱ 依時間 / 👤 依訪客分組）。分組卡顯示訪客代號（依碼上色）、掃描次數、**不同網站數**、最近活動時間，掃過 ≥2 個不同網站標「🔍 多站比較」。上線首日即看出：訪客 `d913` 連掃 `prettyskin.com.tw`＋`plusbeauty.com.tw`（兩個醫美站，疑似醫美業或代理商比較競品，正中 ICP）、`62d6` 3 分鐘內掃 3 個站（含兩個日本網站）。Vite transform 913 ✓。

---

### 2026-07-21（轉換漏斗斷點：儀表板 aivis 區塊只寫「升 Pro」、從沒告訴用戶有 7 天免費試用）

**怎麼發現的**：廣告 7/16 起跑（花 NT$1,366），漏斗 曝光 10,203 → 點擊 240 → 落地頁 210 → 按檢測 76 → **註冊 12（15.8%，健康）→ 付費 0**。查 AdminActivity 發現關鍵：**12 個全部「活躍」、全部有掃描 2–7 次（最多 7 次）、但方案全是 Free、沒有一個啟動試用，最後行為清一色「網站掃描」、沒人碰過 aivis**。→ 問題不是留不住人，是**從免費掃描到 aivis 之間沒有橋**。

**根因**：[DashboardV2.jsx](src/pages/DashboardV2.jsx) `AivisHero` 對免費用戶只顯示「aivis 是 Pro 核心功能…**升 Pro 解鎖**」+ 按鈕導去 `/pricing`。**整段沒有一個字提到「你有 7 天免費試用」**——試用入口只存在於 Pricing / Account / AIVisibility 頁。等於在用戶興趣最高的當下（剛看完自己網站分數）跟他要錢，而不是邀請他免費試。

**修法**：`AivisHero` 取 `hasTrialedBefore / startTrial / isTrial`，對「未付費 ∧ 未試用中 ∧ 從未試用過」的用戶改成試用邀請 —— 標題改「你剛看完『網站體質』——但 AI 到底推不推薦你？」、說明點出掃描 vs aivis 的差別、CTA 改「**免費試用 7 天 → 看 AI 推不推薦我**」直接呼叫 `startTrial()`（不再跳定價頁），附「免費・不用綁卡・隨時可取消」。`startTrial` 回傳的 `error` 是字串代碼，已對應人話訊息（already_trialed / not_authenticated / 其他）。已試用過的用戶維持原本導向 `/pricing`。Vite transform 913 ✓。
...
1. 後台掃描紀錄看不到「嘗試」——LP 按掃描只發 FB Pixel Lead、不寫 DB。
2. Meta 像素 PageView 502 但**被自己 2 天密集測試污染**、不可信。
3. **廣告管理員（乾淨歸因）才是真相**：曝光 5,194 → 連結點擊 329（CTR 6.33%、CPC NT$2）→ 到達 LP 261 → **Lead 0 → 掃描 0**。
4. 結論：廣告好（CTR/CPC 佳、261 人進 LP），死在 **261 人 0 行動**。

**兩個根因 + 修法：**
- **LP 掃描框掉出手機折線**（846px > 844px 第一屏）：訪客只看到大標+範例儀表板，看不到掃描框 → 0 行動。修：[LandingPage.jsx](src/pages/lp/LandingPage.jsx) 掃描框上移到範例儀表板**上面**、縮 logo `mb-14→mb-8`。驗證：掃描框 846→**381px**。
- **掃描前必須登入**（[HomeDark.jsx:436](src/pages/HomeDark.jsx) + input onFocus 踢登入）：value-first 拆牆。未登入也能掃（SEO/AEO/GEO/EEAT 瀏覽器端算、不寫 DB、零 LLM 成本），inline 顯示 4 大分數 teaser；**完整建議/保存/AI 曝光監測(aivis，貴的 LLM)才需免費註冊**。登入流程不變。CTA 存 `lp_pending_url` → /register → 註冊後回首頁帶入網址重掃進完整 dashboard。驗證：Playwright 未登入掃 example.com → 不被踢登入、出現分數面板（SEO89/AEO0/GEO13）、0 JS 錯誤。

**策略取捨（已與用戶確認）：** 放棄「掃描當下抓 email」換「先給價值再引導註冊」。登入牆設在 aivis（成本對齊：免費送便宜的掃描、要錢的 LLM 在牆後）。先做 teaser 版（A）驗證轉換，有效再考慮全套免費（B）。
...
**改動 [DashboardV2.jsx](src/pages/DashboardV2.jsx)：** 把 `<AuditSection>` 從工具箱上方移到 aivis Hero／趨勢圖正下方。新順序：Hero → 趨勢 → **站點體檢** → 達標引導 → 品牌外部提及 → 本週趨勢 → 今日該修 → 工具箱。純位置調整、邏輯不變。`vite build` exit 0。
...
**改動 [AIVisibilityDashboard.jsx](src/pages/AIVisibilityDashboard.jsx)：** 趨勢區塊從兩段變三段——
- `daysWithData = trendData.filter(d=>d.total>0).length`（有掃描的天數，不是次數）。
- 渲染：`isEmpty`（0 天）→ `TrendGhostRadar`（開始監測雷達）｜`daysWithData < 3`（1-2 天）→ **新 `TrendForming`**｜`>=3` 天 → 真 `TrendChart`。
- `TrendForming`：把「今天的真實提及率」放最左當起點（實心發光點 + 雷達漣漪 + 「今天 X%」），往右上投射脈動 dashed「預期走勢」到「30 天後」。前瞻框架（今天→30天後）而非歷史框架——因 30 天歷史圖今天在最右、沒空間往右投射。
- **honesty 防線：** 真數據實心發光、預期走勢一律 dashed + 明標「示意」，不偽裝成真實歷史，只是視覺勾子。「趨勢成形中」LIVE 徽章脈動 + 「再次掃描」鈕（綁 runScan）。
- 踩坑修正：第一版沿用歷史 index 把起點放最右、投射線被擠在右邊 28px → 改前瞻投射。
- 驗證：`vite build` exit 0、bundle 含「30 天後」（rewrite 專屬字串）。純前端、不動後端。

**門檻備註：** `< 3` 天可調；目的是「點太少畫不出趨勢」期間維持勾子，等真有 3+ 天資料自動切回真趨勢圖。
...
**後續收尾（同日，最終狀態）：**
- **文案定位**：[BrandMentionsCard.jsx](src/components/v2/BrandMentionsCard.jsx) 數字標籤「網路提及次數」→「**AI 參考來源**」、副標改「AI 會參考哪些地方在談你的品牌」。理由：講「次數」=暗示精確計數，但接地會跳 → 重蹈 aivis 波動的信任陷阱；改講「來源」（質），小數字／波動就不會被當成不準，反而是「這幾個是 AI 真正在看的關鍵來源、去爭取曝光」的可行動洞察。
- **假 0 浮動**：實測單次掃描約 **4 成回空**（用戶實況：第 1 次 0、第 2 次 4）。三層緩解：(a) 後端回空**重試 2→3 次**（假 0 機率 ~36% → ~10%，只有回空才重試、正常查詢不變慢）；(b) 前端**回 0 不快取**（false 0 不被鎖 24h、再掃就出來）；(c) 文案改「來源」定位。
- **解讀原則**：看到非零=真實來源可信；看到 0=再掃一次確認。
- **根本限制 / 未來**：Gemini 接地本質非確定性（它是「找來源回答問題」非「窮舉搜尋」），**無法做到完全不跳的精準計數**。若日後要「穩定的媒體曝光計數」，正解是換**確定性搜尋 API（Bing Web Search / Brave Search / SerpAPI）**——都沒有 Google CSE 的 403 問題、回固定結果＋數量。先不做，需要精準計數時再評估。
...
**改動 [src/pages/AIVisibilityDashboard.jsx](src/pages/AIVisibilityDashboard.jsx)：**
- 新增 `CitedSourcesCard`（主打卡，全寬、醒目）：彙整最近一次掃描所有引擎的接地來源（去重 + 計數「被幾題引用」），可點連結，並寫清楚可行動洞察「想被 AI 推薦就要出現在這些地方」。`citedSources` useMemo 從 `recentResults` 聚合。
- **版面重構**：原 Prompts + 趨勢「兩欄」拆開 → 趨勢線升為**全寬主角**（放概況卡正下方）、引用來源主打卡、Prompts 全寬。手機版反而更好（原 two-col 無 @media 會擠）。
- **區間**：概況卡「品牌曝光率」sub 改顯示「近 7 天 X–Y%・AI 答案每天在變」（`recentRange` useMemo 取近 7 天有掃描的 min/max；不足或持平則 fallback 原文案）。
- **說明**：趨勢圖副標改「AI 即時上網、答案天天在變 — 看趨勢，別看單次數字」（順手拿掉未接的「每日 02:00 自動掃描」假文案）。

**驗證：** eslint 無新增問題；vite transform 902 模組全過（JSX 合法）。

**待用戶：** push 部署後看新版面（趨勢當主角 + 引用來源主打卡）；引用來源只有新掃描才有。
